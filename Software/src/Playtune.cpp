...
*   30 March 2015, Anderson Li V1.5
*     - Stripped down fir nano. Timer1 is initiallized first
*
*/
...
We currently use timer1, since that one is the first to be allocated.
*/
...
volatile unsigned tune_program_repeat = 0 ;                                /* repat counter*/
volatile float tune_program_frequency[AVAILABLE_TIMERS] = { 0, 0 }; /* current program frequncy*/
volatile float tune_program_modifier[AVAILABLE_TIMERS] = { 0, 0 };         /* pitch modification at each interval*/

volatile boolean Playtune::tune_playing = false;
volatile boolean Playtune::tune_motor_running = false;
...
void tune_playnote (byte chan, byte note, float frequency);
...
void Playtune::tune_initchan(byte pin) {
    ...
    if (_tune_num_chans < AVAILABLE_TIMERS) {
        ...
        case 1:
            ...
            tune_playnote (0, 60, false);  /* start and stop channel 0 (timer 1) on middle C so wait/delay works */
            tune_stopnote (0);
            break;
        }
    }
...
//-----------------------------------------------
// Stops alll the notes without clearing the channels
//-----------------------------------------------
...
void tune_cleanUp(void) {
    ...
    Playtune::tune_motor_running = false; 
...
#define CMD_MOTOR	    0xa0	/* stop running motor */
#define CMD_PROG_NOTE_INIT  0xb0	/* initialize programed note. value is the channel */
#define CMD_PROG_NOTE_PLAY  0xc0        /* play programed note. value is the channel */
#define CMD_PROG_REPEAT     0xd0	/* repeat score. Next value is how many times to repeat if true. how far to move back if false*/
...
//      Serial.println(cmd, HEX);
        ...
        if (opcode == CMD_STOPNOTE) { /* stop note */
            ...
            tune_playnote (value, pgm_read_byte(score_cursor++), false);
        }
        ...
        else if ( opcode == CMD_MOTOR ) { /* start or stop motor*/
            Playtune::tune_motor_running = value; 
        }
        else if ( opcode == CMD_PROG_NOTE_INIT ) { /* programmed sound effect init*/
            param = ((unsigned)cmd << 8) | (pgm_read_byte(score_cursor++));
            tune_program_frequency[value] = pgm_read_word (tune_frequencies2_PGM + param);
            param = ((unsigned)cmd << 8) | (pgm_read_byte(score_cursor++));
            tune_program_modifier[value] =  param/100.0;
        }
        else if ( opcode == CMD_PROG_NOTE_PLAY ) { /* programmed sound effect play*/
            tune_playnote (value, false, tune_program_frequency[value]);
            tune_program_frequency[value] = tune_program_frequency[value] * tune_program_modifier[value];
        }
        else if ( opcode == CMD_PROG_REPEAT ) { /* programmed sound repeat*/
            if ( value ) {
                param = ((unsigned)cmd << 8) | (pgm_read_byte(score_cursor++));
                tune_program_repeat = param; //initialize
            } 
            else {
                tune_program_repeat--;
                if ( tune_program_repeat > 0 ) {
                  param = ((unsigned)cmd << 8) | (pgm_read_byte(score_cursor));
                  score_cursor -= param; // repeat program commands
                   
                } else {
                    score_cursor++; // move on to the next score
                }
            } 
        }
    }
...
void Playtune::tune_disableInterrupts(void) {
    tune_cleanUp();
}
...
ISR(TIMER1_COMPA_vect) {  // TIMER 1
    ...
    // Timer 1 is the one assigned first, so we keep it running always
